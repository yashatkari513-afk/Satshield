...
try:
    ...
    from satshield_ml.explainability import build_explainability_report
...
class SatelliteAnomalyPredictor:
    ...
    def _prepare_features(self, sat_id: str, raw_data: Dict[str, float]) -> pd.DataFrame:
        ...
        for k in RAW_TELEMETRY_FIELDS:
            base_name = k.replace('_c', '').replace('_v', '').replace('_a', '').replace('_percent', '').replace('_w', '').replace('_db', '').replace('_g', '').replace('_deg', '')
            nominal_ref = BASELINE_REFS.get(k, 0.0)
            if len(window_df) > 1:
                r_mean = float(window_df[k].mean())
                r_std = float(window_df[k].std())
                dev = current_raw[k] - r_mean
            else:
                r_mean = nominal_ref
                r_std = 0.1
                dev = current_raw[k] - nominal_ref
            
            features[f"{base_name}_deviation"] = round(dev, 4)
    ...
    def predict(self, telemetry: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 4. Explainable AI Root-Cause & Evidence Layer (Step 10)
        severity = "CRITICAL" if prediction == "ANOMALY" and raw_score < -0.05 else ("WARNING" if prediction == "ANOMALY" else "INFO")
        xai_report = build_explainability_report(
        ...
        evidence_strings = [
            f"{item['parameter']} ({item['value']} {item['unit']}) {item['status'].replace('_', ' ').lower()} by {item['deviation']}"
            for item in xai_report['evidence']
        ]
        ...
        return {
            ...
            "subsystem": xai_report['subsystem'],
            "anomaly_type": xai_report['anomaly_type'],
            "severity": severity,
            "model_type": "IsolationForest",
            "model_status": "trained_model_loaded",
            "feature_count": len(self.feature_columns),
            ...
            "evidence_items": xai_report['evidence'],
            "contributing_factors": xai_report['contributing_factors'],
            "probable_root_cause": xai_report['probable_root_cause'],
            "mission_impact": xai_report['mission_impact'],
            "recommended_action": xai_report['recommended_action'],
            "explanation": xai_report['probable_root_cause'],
            "explanation_method": xai_report['explanation_method'],
