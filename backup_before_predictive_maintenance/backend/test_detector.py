...
def run_tests():
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        print(f"  * Affected Telemetry: {len(result['affected_telemetry'])} parameter(s) deviated")
        
        if result['affected_telemetry']:
            for param, details in result['affected_telemetry'].items():
