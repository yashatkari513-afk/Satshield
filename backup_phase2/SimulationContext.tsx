...
export const SimulationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Helper to extract ML telemetry payload
  const getPayloadFromSat = useCallback((satData: SatelliteTelemetryMap, satId: string): MLTelemetryInput => {
    const current = satData[satId] || satData[Object.keys(satData)[0]];
  ...
  // Trigger ML inference on backend
  ...
  const buildDynamicDiagnosis = useCallback((
    ...
    if (normalizedKey.includes('temp') || normalizedKey.includes('heat')) {
      ...
      return {
        ...
        trendTrajectory: 'Increasing risk',
        persistence: '4 consecutive observation frames exceeding +2.5σ',
        rateOfChange: '+1.85 °C/min',
        timeToFailureEstimate: 'Not yet validated (predictive maintenance model pending)',
        observationWindow: '6-frame telemetry sequence (400s rolling window)',
        trend: `Trend trajectory: Divergent thermal elevation observed across consecutive telemetry frames`,
    ...
    if (normalizedKey.includes('comm') || normalizedKey.includes('signal') || normalizedKey.includes('packet')) {
      ...
      return {
        ...
        trendStatus: 'DEGRADING',
        trendTrajectory: 'Increasing risk',
        persistence: '3 consecutive ground pass sweeps outside margin',
        rateOfChange: '-4.2 dBm/min',
        timeToFailureEstimate: 'Not yet validated (predictive maintenance model pending)',
        observationWindow: '5-frame pass sequence (300s window)',
        trend: `Trend trajectory: Carrier link margin degradation observed across pass window`,
        recommendedAction: operationalActions.immediate,
        operationalActions,
    ...
    if (normalizedKey.includes('attitude') || normalizedKey.includes('instability') || normalizedKey.includes('drift')) {
      ...
      return {
        ...
        trendStatus: 'DEGRADING',
        trendTrajectory: 'Increasing risk',
        persistence: '5 consecutive AOCS control loop cycles outside envelope',
        rateOfChange: '+0.85 deg/min',
        timeToFailureEstimate: 'Not yet validated (predictive maintenance model pending)',
        observationWindow: '6-cycle telemetry buffer (360s window)',
        trend: `Trend trajectory: Angular pointing deviation increasing over orbital arc`,
        recommendedAction: operationalActions.immediate,
        operationalActions,
    ...
    if (normalizedKey.includes('solar')) {
      ...
      return {
        ...
        trendStatus: 'DEGRADING',
        trendTrajectory: 'Increasing risk',
        persistence: '4 daylight pass samples outside normal envelope',
        rateOfChange: '-35.0 W/min',
        timeToFailureEstimate: 'Not yet validated (predictive maintenance model pending)',
        observationWindow: '6-frame telemetry buffer (450s window)',
        trend: `Trend trajectory: Solar power output collapse observed across daylight passes`,
        recommendedAction: operationalActions.immediate,
        operationalActions,
    ...
    if (normalizedKey.includes('sensor')) {
      ...
      return {
        ...
        persistence: '6 consecutive frames with persistent monotonic bias',
        rateOfChange: '+0.06 V/hour',
        timeToFailureEstimate: 'Not yet validated (predictive maintenance model pending)',
        observationWindow: '6-frame rolling sequence (600s window)',
        trend: `Trend trajectory: Monotonic sensor calibration bias detected across 6 frames`,
        recommendedAction: operationalActions.immediate,
        operationalActions,
    ...
    return {
      ...
      trendStatus: 'DEGRADING',
      trendTrajectory: 'Increasing risk',
      persistence: '5 consecutive observation frames outside margin',
      rateOfChange: '-0.45 V/min',
      timeToFailureEstimate: 'Not yet validated (predictive maintenance model pending)',
      observationWindow: '6-frame telemetry sequence (420s window)',
      trend: `Trend trajectory: Progressive discharge voltage decay observed across telemetry window`,
      recommendedAction: operationalActions.immediate,
      operationalActions,
  ...
  const startAnomalySimulation = useCallback((
    targetSatId?: string,
    scenarioKey: string = 'battery_degradation',
