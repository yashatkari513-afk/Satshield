...
export const AIDiagnosisModal: React.FC = () => {
  ...
  const anomalyScoreDisplay = currentDiagnosis?.anomalyScore || (latestMLResult ? latestMLResult.anomaly_score.toFixed(4) : '0.0000');
  const detectionConfidence = currentDiagnosis?.detectionConfidence || (latestMLResult && latestMLResult.confidence ? `${latestMLResult.confidence.toFixed(1)}% Calibrated` : 'Confidence: Not calibrated');
  const detectionMethod = currentDiagnosis?.detectionMethod || (latestMLResult ? 'Isolation Forest (scikit-learn)' : 'Rule-based baseline detector');
  const decisionThreshold = currentDiagnosis?.decisionThreshold || '0.6500';
  const trendStatus = currentDiagnosis?.trendStatus || (currentSat.status === 'CRITICAL' ? 'DEGRADING' : 'STABLE');
  const trendTrajectory = currentDiagnosis?.trendTrajectory || (currentSat.status === 'CRITICAL' ? 'Increasing risk' : 'Stable');
  const persistenceText = currentDiagnosis?.persistence || 'Single frame observation';
  const rateOfChangeText = currentDiagnosis?.rateOfChange || 'Nominal rate of change';
  const observationWindow = currentDiagnosis?.observationWindow || '6-frame telemetry buffer';
  const timeToFailureText = currentDiagnosis?.timeToFailureEstimate || 'Not yet validated';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn select-none">
      ...
      >
        ...
        <div className="p-4 rounded-xl bg-[#080808] border border-white/15 mb-4">
          ...
          {telemetryEvidenceList.length > 0 ? (
            ...
            <div className="text-[11.5px] text-[#94A3B8] p-3 bg-black/50 rounded-lg border border-white/10">
              Telemetry streams on <strong className="text-white">{targetSatName}</strong> are being processed through the real-time threshold and anomaly baseline matrix.
            </div>
          )}
        </div>

        {/* ── 4 & 5. AI DECISION & TEMPORAL TREND ANALYSIS (2 COLUMNS) ── */}
        ...
        {/* ── 6. PROBABLE ROOT CAUSE (RANKED QUALITATIVE) ── */}
        <div className="p-4 rounded-xl bg-[#080808] border border-white/15 mb-4">
          <div className="flex items-center justify-between gap-2 mb-2.5">
            ...
            <span className="text-[9.5px] text-[#94A3B8] italic">
              Probable causes — based on telemetry evidence
            </span>
          </div>
          ...
          {probableCausesList.length > 0 ? (
            ...
            <div className="text-[11px] text-[#94A3B8] p-2.5 bg-black/50 rounded-lg">
              Telemetry parameters indicate nominal system equilibrium across all physical domains.
            </div>
          )}
        </div>
