...
export interface MLContributingFactor {
  type: 'PRIMARY' | 'SUPPORTING' | string;
  title: string;
  description: string;
}

export interface MLExplainabilityReport {
  satellite_id: string;
  anomaly_type: string;
  subsystem: string;
  ...
  contributing_factors: MLContributingFactor[];
...
export interface MLRootCauseAnalysis {
  satellite_id?: string;
  affected_subsystem: string;
  primary_probable_cause: string;
  evidence: string[];
  contributing_factors: string[];
  evidence_strength: 'STRONG' | 'MODERATE' | 'WEAK' | 'AMBIGUOUS' | 'NOMINAL' | string;
  severity: 'CRITICAL' | 'HIGH' | 'MEDIUM' | 'LOW' | 'NOMINAL' | string;
  persistence: string;
...
export interface MLMissionImpactAnalysis {
  satellite_id?: string;
  mission_impact_level: 'NOMINAL' | 'LOW' | 'MODERATE' | 'HIGH' | 'CRITICAL' | string;
  affected_capabilities: string[];
  primary_operational_impact: string;
  potential_mission_consequences: string[];
  impacted_subsystems: string[];
  risk_drivers: string[];
  persistence: string;
  trend_summary: string;
  urgency: 'ROUTINE' | 'WATCH' | 'ACTION REQUIRED' | 'IMMEDIATE FLIGHT INTERVENTION' | string;
  recommended_operator_response: string[];
  root_cause_context?: Record<string, any>;
  data_quality?: string;
  analysis_method?: string;
  limitations?: string;
}
...
export interface MLAnomalyResponse {
  ...
  recommended_action: string;
  explanation_method?: string;
  explainability?: MLExplainabilityReport;
  root_cause_analysis?: MLRootCauseAnalysis;
  mission_impact_analysis?: MLMissionImpactAnalysis;
  predictive_maintenance?: Record<string, any>;
  risk_level?: string;
  early_warning?: string;
  estimated_time_to_threshold?: number;
