...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  // Authoritative single-source active alert calculation synchronized with Alerts page
  const activeAlerts: DynamicAlert[] = useMemo(() => {
          ...
          >
            ...
            <span
              className={`absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full text-[9px] font-bold flex items-center justify-center ${
                activeAlerts.length > 0 ? 'bg-[#EF4444] text-white' : 'bg-slate-700 text-slate-300'
              }`}
              style={{ boxShadow: activeAlerts.length > 0 ? '0 0 6px rgba(239, 68, 68, 0.7)' : 'none' }}
            >
              {activeAlerts.length}
            </span>
          </button>
          ...
          {notificationsOpen && (
            ...
            >
              <div className="flex items-center justify-between pb-2 border-b border-white/10 font-sans text-[11px] font-bold text-[#F1F5F9]">
                <span>ACTIVE FLEET ALERTS ({activeAlerts.length})</span>
                <button
                  type="button"
              ...
              <div className="mt-2 max-h-60 overflow-y-auto space-y-2 custom-scrollbar">
                {activeAlerts.length === 0 ? (
                  <div className="p-4 text-center text-[10.5px] font-mono text-emerald-400">
                  ...
                  activeAlerts.slice(0, 4).map((alert) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[#F1F5F9]">{alert.satellite_name}</span>
                        <span className="text-[9px] font-bold text-[#EF4444]">{alert.severity}</span>
                      </div>
                      <p className="text-[#94A3B8] text-[10px] mt-0.5">{alert.message}</p>
                    </div>
                  ))
                )}
              </div>
