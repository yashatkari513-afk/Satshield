...
import { BeforeAfterReportService } from '../services/BeforeAfterReportService';
...
export interface ActiveDiagnosis {
  ...
  contributingFactors?: { type: string; title: string; description: string }[];
...
export const SimulationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const buildDynamicDiagnosis = useCallback((
    ...
    if (
      ...
      return {
        ...
        contributingFactors: mlDet?.contributing_factors || [
          {
            type: 'PRIMARY',
            title: 'Nominal Telemetry Baseline',
            description: 'All 42 monitored telemetry features tracking within established multi-dimensional bounds.'
          }
        ],
        probableRootCause: mlDet?.probable_root_cause || 'Nominal telemetry. No significant contributing anomaly factors detected.',
        explanationMethod: mlDet?.explanation_method || 'IsolationForest 42-Feature Empirical Grounding',
        dataQuality: mlDet?.data_quality || 'GOOD',
      };
    }
    ...
    if (normalizedKey.includes('temp') || normalizedKey.includes('heat')) {
      ...
      return {
        ...
        contributingFactors: mlDet?.contributing_factors || [
          {
            type: 'PRIMARY',
            title: `Elevated Core Temperature (${currentTemp.toFixed(1)}°C)`,
            description: `Core thermal envelope exceeds learned nominal baseline (24.0°C) by +${baselineDiff}°C.`
          },
          {
            type: 'SUPPORTING',
            title: 'Thermal Dissipation Gradient',
            description: 'Elevated dT/dt rate of change indicates active heat dissipation imbalance.'
          }
        ],
        probableRootCause: mlDet?.probable_root_cause || `Thermal radiator surface occlusion or payload power dissipation excess is suspected because core temperature reached ${currentTemp.toFixed(1)}°C (+${baselineDiff}°C deviation).`,
        explanationMethod: mlDet?.explanation_method || 'IsolationForest 42-Feature Empirical Grounding',
        dataQuality: mlDet?.data_quality || 'GOOD',
      };
    }
    ...
    if (normalizedKey.includes('comm') || normalizedKey.includes('signal') || normalizedKey.includes('packet')) {
      ...
      return {
        ...
        contributingFactors: mlDet?.contributing_factors || [
          {
            type: 'PRIMARY',
            title: `Degraded RF Carrier Signal (${sigVal.toFixed(1)} dBm)`,
            description: `Downlink carrier signal dropped by ${(sigVal + 75).toFixed(1)} dBm relative to nominal reference (-75.0 dBm).`
          },
          {
            type: 'SUPPORTING',
            title: 'Telemetry Packet Drop Rate',
            description: `Packet CRC error rate elevated to ${packetLossVal.toFixed(1)}% during contact window.`
          }
        ],
        probableRootCause: mlDet?.probable_root_cause || `RF downlink carrier attenuation or ground-station autotracking misalignment is suspected because carrier link signal dropped to ${sigVal.toFixed(1)} dBm.`,
        explanationMethod: mlDet?.explanation_method || 'IsolationForest 42-Feature Empirical Grounding',
        dataQuality: mlDet?.data_quality || 'GOOD',
      };
    }
    ...
    if (normalizedKey.includes('attitude') || normalizedKey.includes('instability') || normalizedKey.includes('drift')) {
      ...
      return {
        ...
        contributingFactors: mlDet?.contributing_factors || [
          {
            type: 'PRIMARY',
            title: 'Attitude Pointing Deviation (±5.8°)',
            description: '3-axis boresight pointing deviation exceeds operational fine-pointing threshold (±0.08°).'
          },
          {
            type: 'SUPPORTING',
            title: 'Reaction Wheel Speed Saturation',
            description: 'Reaction wheel #2 RPM approaching maximum angular momentum threshold.'
          }
        ],
        probableRootCause: mlDet?.probable_root_cause || 'Reaction wheel angular momentum accumulation or AOCS pointing drift is suspected because 3-axis pointing deviation error exceeds tolerance boundaries.',
        explanationMethod: mlDet?.explanation_method || 'IsolationForest 42-Feature Empirical Grounding',
        dataQuality: mlDet?.data_quality || 'GOOD',
      };
    }
    ...
    if (normalizedKey.includes('solar')) {
      ...
      return {
        ...
        contributingFactors: mlDet?.contributing_factors || [
          {
            type: 'PRIMARY',
            title: `Solar Power Output Drop (${powerWatts}W)`,
            description: `Array generation collapsed by ${(((tel.power * 7.5 - 650) / 650) * 100).toFixed(1)}% relative to 650W nominal daylight reference.`
          },
          {
            type: 'SUPPORTING',
            title: 'SADA Gimbal Sun Vector Error',
            description: 'Solar array gimbal tracking lag detected relative to optimal illumination vector.'
          }
        ],
        probableRootCause: mlDet?.probable_root_cause || `Solar array drive stepping lag or partial occlusion is suspected because array power output dropped to ${powerWatts}W during daylight pass.`,
        explanationMethod: mlDet?.explanation_method || 'IsolationForest 42-Feature Empirical Grounding',
        dataQuality: mlDet?.data_quality || 'GOOD',
      };
    }
    ...
    return {
      ...
      contributingFactors: mlDet?.contributing_factors || [
        {
          type: 'PRIMARY',
          title: `EPS Main Bus Undervoltage (${voltVal}V)`,
          description: `Power bus voltage dropped by ${(((tel.voltage - 28.0) / 28.0) * 100).toFixed(1)}% relative to nominal 28.0V baseline.`
        },
        {
          type: 'SUPPORTING',
          title: `Battery Reserve Depletion (${battVal}%)`,
          description: 'State of charge depleted below operational margin with elevated discharge draw.'
        }
      ],
      probableRootCause: mlDet?.probable_root_cause || `Battery electrochemical cell degradation or main bus undervoltage is suspected because bus voltage dropped to ${voltVal}V while battery SOC depleted to ${battVal}%.`,
      explanationMethod: mlDet?.explanation_method || 'IsolationForest 42-Feature Empirical Grounding',
      dataQuality: mlDet?.data_quality || 'GOOD',
    };
  }, []);
  ...
  const startAnomalySimulation = useCallback((
    ...
    // Capture baseline BEFORE snapshot before starting anomaly mutation
    const currentSatData = satellitesData[satId] || INITIAL_BASELINE_MAP[satId];
    if (currentSatData && currentSatData.telemetry) {
      BeforeAfterReportService.captureBefore(
        satId,
        currentSatData.telemetry,
        currentSatData.telemetry.healthScore || 95,
        'NOMINAL',
        'NORMAL',
        0.08
      );
    ...
    const t5 = setTimeout(() => {
      ...
      setSatellitesData((latestSats) => {
        const targetLatest = latestSats[satId] || latestSats['SAT-001'];
        if (targetLatest) {
          const diag = buildDynamicDiagnosis(targetLatest, normalizedKey, severityTarget);
          BeforeAfterReportService.captureAfter(
            satId,
            targetLatest.telemetry,
            diag
          );
        }
        return latestSats;
  ...
  }, [activeSatelliteId, buildDynamicDiagnosis, resetDemo, satellitesData]);
