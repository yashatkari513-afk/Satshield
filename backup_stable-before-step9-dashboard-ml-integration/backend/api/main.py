...
async def trigger_anomaly_scenario(payload: ScenarioTriggerRequest):
    ...
    det_response = AnomalyDetectionResponse(
        ...
        explanation=ml_res.get("explanation") or f"IsolationForest evaluation on {sat_id}.",
        probable_causes=[sc_meta["description"]] if is_anom else [],
        recommended_action="Execute operational mitigation procedure." if is_anom else "Maintain routine monitoring.",
...
async def detect_anomaly(telemetry: TelemetryInput):
    ...
    try:
        ...
        return AnomalyDetectionResponse(
            ...
            affected_telemetry=[],
            evidence=ml_res.get("evidence", []),
            explanation=ml_res.get("explanation", "Nominal operating envelope."),
            probable_causes=[ml_res.get("explanation")] if is_anom else [],
            recommended_action="Inspect subsystem telemetry and execute operational contingency plan." if is_anom else "Maintain routine monitoring.",
            timestamp=now_utc,
            model="IsolationForest",
