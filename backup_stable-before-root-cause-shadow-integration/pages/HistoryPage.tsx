...
import { exportTelemetryCSV, generatePDFReport } from '../services/ReportService';
...
export const HistoryPage: React.FC = () => {
  ...
  const [selectedSatId, setSelectedSatId] = useState<string>(activeSatellite.id || 'SAT-001');
  const [reports, setReports] = useState<ApiReport[]>([]);
  const [loadingReport, setLoadingReport] = useState<boolean>(false);
  const [activeTab, setActiveTab] = useState<'reports' | 'telemetry_analytics'>('reports');
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#020305] font-sans">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#94A3B8] mt-0.5 font-medium">
              Generate certified 9-section technical health reports, export telemetry datasets, and dispatch to authorized space agencies.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button
            type="button"
            onClick={() => exportTelemetryCSV(currentSat)}
            className="px-3 py-1.5 bg-[#111111] hover:bg-white/10 border border-white/25 text-[#F1F5F9] font-bold text-xs rounded-lg transition-all flex items-center gap-2 cursor-pointer uppercase"
          >
            <Download className="w-3.5 h-3.5 text-[#00BFFF]" /> EXPORT CSV
          </button>
      ...
      {/* ── TAB SELECTOR: REPORT CENTER TABLE vs HISTORICAL CHARTS ── */}
