...
export const AIDiagnosisModal: React.FC = () => {
  ...
  const rootCause = latestMLResult?.root_cause_analysis;
  const shadowSubsystem = rootCause?.affected_subsystem || subsystemName;
  const shadowStrength = rootCause?.evidence_strength || 'MODERATE';
  const shadowEvidence = rootCause?.evidence || [];
  const shadowFactors = rootCause?.contributing_factors || [];
  const shadowCause = rootCause?.primary_probable_cause || probableRootCauseText;

  const sparklineData = currentDiagnosis?.historySparkline;

  // Step 23 Mission Impact Assessment Shadow Layer Extraction
  const miAnalysis = latestMLResult?.mission_impact_analysis;
  const miLevel = (miAnalysis?.mission_impact_level || (severityLevel === 'CRITICAL' ? 'HIGH' : (severityLevel === 'WARNING' ? 'MODERATE' : 'NOMINAL'))).toUpperCase();
  const miUrgency = miAnalysis?.urgency || (miLevel === 'CRITICAL' ? 'IMMEDIATE FLIGHT INTERVENTION' : (miLevel === 'HIGH' ? 'ACTION REQUIRED' : (miLevel === 'MODERATE' ? 'WATCH' : 'ROUTINE')));
  const miCapabilities = miAnalysis?.affected_capabilities && miAnalysis.affected_capabilities.length > 0
    ? miAnalysis.affected_capabilities
    : (missionImpact?.affectedCapability ? [missionImpact.affectedCapability] : ['POWER AVAILABILITY']);
  const miSubsystems = miAnalysis?.impacted_subsystems && miAnalysis.impacted_subsystems.length > 0
    ? miAnalysis.impacted_subsystems
    : [subsystemName];
  const miPrimaryImpact = miAnalysis?.primary_operational_impact || (missionImpact?.potentialConsequence || 'Subsystem telemetry operating within evaluated margins.');
  const miConsequences = miAnalysis?.potential_mission_consequences && miAnalysis.potential_mission_consequences.length > 0
    ? miAnalysis.potential_mission_consequences
    : (missionImpact?.potentialConsequence ? [missionImpact.potentialConsequence] : []);
  const miRiskDrivers = miAnalysis?.risk_drivers && miAnalysis.risk_drivers.length > 0
    ? miAnalysis.risk_drivers
    : [];
  const miResponses = miAnalysis?.recommended_operator_response && miAnalysis.recommended_operator_response.length > 0
    ? miAnalysis.recommended_operator_response
    : [];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/85 backdrop-blur-md animate-fadeIn select-none">
      ...
      >
        ...
        {/* ── 7. ROOT-CAUSE ANALYSIS (SHADOW MODE INTEGRATION) ── */}
        <div className="p-4 rounded-xl bg-[#080808] border border-white/15 mb-4">
          <div className="flex items-center justify-between gap-2 mb-2.5">
            <div className="flex items-center gap-2 text-[11px] font-bold text-[#F1F5F9] uppercase tracking-wider">
              <Layers className="w-3.5 h-3.5 text-cyan-400" />
              <span>ROOT-CAUSE ANALYSIS</span>
              <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-cyan-950/50 text-cyan-300 border border-cyan-500/30 uppercase tracking-wide">
                SHADOW LAYER
              </span>
            </div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 rounded text-[9.5px] font-black uppercase tracking-wider bg-black text-amber-300 border border-amber-500/30">
                {shadowSubsystem}
              </span>
            </div>
          </div>
          ...
          <div className="p-3 rounded-lg bg-black/80 border border-white/10 mb-3 text-[11.5px] text-slate-200 leading-relaxed font-medium">
            <div className="flex items-center justify-between mb-1.5">
              <strong className="text-cyan-300 uppercase text-[10px] font-mono tracking-wider">
                Telemetry-Grounded Probable Cause:
              </strong>
              {rootCause?.persistence && (
                <span className="text-[10px] text-[#94A3B8] font-mono">
                  Persistence: <strong className="text-white">{rootCause.persistence}</strong>
                </span>
              )}
            </div>
            <p className="text-white font-semibold text-[12px]">{shadowCause}</p>
            {rootCause?.trend_summary && (
              <div className="mt-1.5 text-[10.5px] text-[#94A3B8]">
                <span className="text-cyan-400 font-medium">Trend Dynamics:</span> {rootCause.trend_summary}
              </div>
            )}
          </div>

          {/* Supporting Telemetry Evidence */}
          {shadowEvidence.length > 0 && (
            <div className="mb-3 space-y-1.5">
              <div className="text-[10px] font-mono uppercase text-[#94A3B8] tracking-wider font-bold">
                Supporting Telemetry Evidence:
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {shadowEvidence.map((ev, eIdx) => (
                  <div
                    key={eIdx}
                    className="p-2 rounded bg-black/60 border border-white/5 text-[11px] text-slate-300 flex items-start gap-2"
                  >
          ...
          {shadowFactors.length > 0 && (
            <div className="mb-2 flex flex-wrap items-center gap-1.5">
              <span className="text-[10px] font-mono uppercase text-[#94A3B8] mr-1 font-bold">
                Contributing Factors:
              </span>
              {shadowFactors.map((f, fIdx) => (
                <span
                  key={fIdx}
                  className="px-2 py-0.5 rounded bg-white/5 border border-white/10 text-[10px] font-medium text-slate-300"
                >
                  {f}
                </span>
              ))}
          ...
          {/* Fallback to Probable Causes List if provided */}
          {probableCausesList.length > 0 && shadowEvidence.length === 0 && (
          ...
          <div className="mt-2.5 pt-2 border-t border-white/10 text-[9.5px] text-[#94A3B8] italic flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <span>
              Telemetry-grounded probable root cause based on multi-signal correlation — not confirmed hardware failure.
            </span>
            <span className="font-mono text-cyan-400/80 shrink-0">
              Method: Multi-Signal Physical Correlation Matrix
            </span>
          </div>
        </div>

        {/* ── 7. MISSION IMPACT ASSESSMENT (STEP 23 SHADOW LAYER) ── */}
        <div className="p-4 rounded-xl bg-[#080808] border border-white/15 mb-4">
          <div className="flex flex-wrap items-center justify-between gap-2 mb-2.5">
            <div className="flex items-center gap-2 text-[11px] font-bold text-[#F1F5F9] uppercase tracking-wider">
              <AlertTriangle className="w-3.5 h-3.5 text-amber-400" />
              <span>MISSION IMPACT ASSESSMENT</span>
              <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-amber-950/50 text-amber-300 border border-amber-500/30 uppercase tracking-wide">
                OPERATIONAL RISK SHADOW
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5 rounded text-[9.5px] font-black uppercase tracking-wider border ${urgencyBadgeStyle}`}>
                URGENCY: {miUrgency}
              </span>
              <span className={`px-2 py-0.5 rounded text-[9.5px] font-black uppercase tracking-wider border ${miBadgeStyle}`}>
                IMPACT: {miLevel}
              </span>
            </div>
          </div>

          {/* Primary Operational Impact & Affected Capabilities */}
          <div className="p-3 rounded-lg bg-black/80 border border-white/10 mb-3 text-[11.5px] leading-relaxed">
            <div className="flex flex-wrap items-center justify-between gap-1 mb-1.5">
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-[10px] font-mono uppercase text-[#94A3B8] font-bold">Affected Capabilities:</span>
                {miCapabilities.map((cap, cIdx) => (
                  <span
                    key={cIdx}
                    className="px-2 py-0.5 rounded bg-white/10 text-white font-bold text-[10px] font-mono border border-white/15"
                  >
                    {cap}
                  </span>
                ))}
              </div>
              {miAnalysis?.persistence && (
                <span className="text-[10px] text-[#94A3B8] font-mono">
                  State: <strong className="text-white">{miAnalysis.persistence}</strong>
                </span>
              )}
            </div>
            <p className="text-white font-semibold text-[12px]">{miPrimaryImpact}</p>
            {miAnalysis?.trend_summary && (
              <div className="mt-1.5 text-[10.5px] text-[#94A3B8]">
                <span className="text-amber-400 font-medium">Temporal Trend:</span> {miAnalysis.trend_summary}
              </div>
            )}
          </div>

          {/* Potential Mission Consequences */}
          {miConsequences.length > 0 && (
            <div className="mb-3 space-y-1.5">
              <div className="text-[10px] font-mono uppercase text-[#94A3B8] tracking-wider font-bold">
                Potential Mission Consequences:
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {miConsequences.map((cons, cIdx) => (
                  <div
                    key={cIdx}
                    className="p-2 rounded bg-black/60 border border-white/5 text-[11px] text-slate-300 flex items-start gap-2"
                  >
          ...
          {/* Risk Drivers & Recommended Operator Response */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5 mb-2">
            {miRiskDrivers.length > 0 && (
              <div className="p-2.5 rounded-lg bg-black/60 border border-white/10">
                <div className="text-[10px] font-mono uppercase text-[#94A3B8] font-bold mb-1">
                  Observed Risk Drivers:
                </div>
                <ul className="space-y-1 text-[11px] text-slate-300">
                  {miRiskDrivers.map((rd, rIdx) => (
            ...
            {miResponses.length > 0 && (
              <div className="p-2.5 rounded-lg bg-black/60 border border-white/10">
                ...
                <ul className="space-y-1 text-[11px] text-slate-200 font-medium">
                  {miResponses.map((resp, respIdx) => (
                    <li key={respIdx} className="flex items-start gap-1.5">
          ...
          <div className="mt-2.5 pt-2 border-t border-white/10 text-[9.5px] text-[#94A3B8] italic flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <span>
              Telemetry-grounded operational risk assessment for decision support — does not represent a validated flight-certification model.
            </span>
            <span className="font-mono text-amber-400/80 shrink-0">
              Method: Deterministic Subsystem Capability & Mission Dependency Matrix
            </span>
          </div>
        </div>
