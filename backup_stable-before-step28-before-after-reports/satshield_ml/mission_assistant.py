...
from satshield_ml.what_if_analysis import analyze_what_if, WhatIfScenarioEngine
...
def classify_question_intent(question: str) -> str:
    ...
    # 1b. What-If Scenario questions
    if re.search(r'\b(what\s*if|what\s+happens\s+if|if\s+.*(continues?|persists?|keeps?|falls?|drops?|rises?|increases?)|will\s+.*cross|project\s*trend|scenario\s*analysis|what\s+happens\s+to\s+mission\s+risk)\b', q):
...
class AIMissionAssistant:
    ...
    def answer_question(
        ...
        if intent == "COMMAND_REQUEST":
            ...
            ans, ev, what_if_data = self._handle_what_if_scenario(question, context)
        ...
        if what_if_data:
            res["what_if_scenario"] = what_if_data
        return res
    ...
    def _handle_what_if_scenario(self, question: str, ctx: Dict[str, Any]) -> Tuple[str, List[str], Dict[str, Any]]:
        sat_id = ctx['satellite_id']
        sat = ctx['satellite_name']
        q_lower = question.lower()

        # Identify requested parameter from natural query text
        param_target = None
        if 'temp' in q_lower or 'heat' in q_lower or 'thermal' in q_lower:
            param_target = 'temperature_c'
        elif 'soc' in q_lower or 'battery' in q_lower or 'charge' in q_lower:
            param_target = 'battery_soc_percent'
        elif 'volt' in q_lower or 'eps' in q_lower:
            param_target = 'voltage_v'
        elif 'current' in q_lower or 'amp' in q_lower:
            param_target = 'current_a'
        elif 'solar' in q_lower or 'sun' in q_lower:
            param_target = 'solar_power_w'
        elif 'signal' in q_lower or 'comm' in q_lower or 'rf' in q_lower or 'downlink' in q_lower:
            param_target = 'communication_signal_db'
        elif 'attitude' in q_lower or 'pointing' in q_lower or 'drift' in q_lower or 'gyro' in q_lower:
            param_target = 'attitude_error_deg'
        elif 'vibration' in q_lower or 'jitter' in q_lower:
            param_target = 'vibration_g'

        raw_tel = ctx.get('raw_payload', {})
        res = analyze_what_if(
            satellite_id=sat_id,
            parameter=param_target,
            current_telemetry=raw_tel,
            projection_horizon_minutes=10.0
        )

        p_name = res.get('parameter', 'Telemetry')
        p_val = res.get('current_value')
        p_unit = res.get('unit', '')
        p_proj = res.get('projected_value')
        p_thresh = res.get('operational_threshold')
        p_risk = res.get('projected_risk', 'NOMINAL')
        p_status = res.get('scenario_status', 'NOMINAL')
        ett = res.get('estimated_time_formatted', 'N/A')
        slope_fmt = res.get('trend_slope_formatted', '0.000 /min')
        why_text = res.get('explanation', '')
        impact_text = res.get('impact', '')
        action_text = res.get('recommended_action', '')

        if p_status == 'INSUFFICIENT_DATA':
            ans = f"What-If Scenario Projection for {sat}:\n\n{why_text}"
            return ans, ["Insufficient telemetry buffer for trend projection."], res

        if p_status == 'THRESHOLD_ALREADY_CROSSED':
            ans = (
                f"What-If Scenario Assessment for {sat}:\n\n"
                f"• Condition: THRESHOLD ALREADY CROSSED\n"
                f"• Current Reading: {p_val} {p_unit} (Operational Threshold: {p_thresh} {p_unit})\n"
                f"• Projected Risk: {p_risk}\n\n"
                f"Why:\n{why_text}\n\n"
                f"Operational Impact:\n{impact_text}\n\n"
                f"Recommended Action:\n• {action_text}"
            )
            return ans, [f"Critical threshold exceeded on {p_name} ({p_val} {p_unit})"], res

        ans = (
            f"What-If Scenario Projection for {sat} (T+10m Horizon):\n\n"
            f"• Parameter: {p_name}\n"
            f"• Current Value: {p_val} {p_unit} (Trend: {slope_fmt})\n"
            f"• Projected Value at T+10m: {p_proj} {p_unit}\n"
            f"• Operational Threshold: {p_thresh} {p_unit}\n"
            f"• Estimated Time to Threshold: {ett}\n"
            f"• Projected Operational Risk: {p_risk}\n\n"
            f"Why:\n{why_text}\n\n"
            f"Operational Impact:\n{impact_text}\n\n"
            f"Recommended Operator Action:\n• {action_text}"
        )
        return ans, [f"{p_name}: Current={p_val}{p_unit}, Projected={p_proj}{p_unit} ({slope_fmt})"], res
